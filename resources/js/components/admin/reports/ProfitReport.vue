                ...
                <div class="card" ref="report">
                    ...
                    <div class="card-body table-responsive p-0" v-if="reportData">
                        <table  v-if="!isLoading" class="table table-hover table-responsive table-bordered">
                            ...
                            <tbody>
                                <tr v-for="(order) in reportData" :key="order.order_id">
                                    <td>{{order.order_id}}</td>
                                    <td>{{order.user_name}}</td>
                                    <td>{{order.created_at | myDate }}</td>
                                    <td>{{ order.amount !=null ?  order.amount: 'NA'}}</td>
                                    <td>{{ order.vendor_total !=null ?  order.vendor_total: 'NA'}}</td>
                                    <td>{{ order.delivery_charges !=null ?  order.delivery_charges: 'NA'}}</td>
                                    <td>{{ order.marketing_charges !=null ?  order.marketing_charges: 'NA'}}</td>
                                    <td> {{ order.profit !=null ?  order.profit: 'NA'}}</td>
                                    <td v-if="showAction">
                                    ...
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <!-- <button product_id="submit" class="btn btn-primary">Save </button> -->
                                            </div>
                                    </div>
                                </div>
                            </div>
...
export default {
    ...
    methods :{
        ...
        getReportData(){
            ...
            this.form.post('/api/report/profit').then( ({data})=>{
        ...
        getProfitLoss(order, index){

            let vendor_total =  order.vendor_total != null && !isNaN(order.vendor_total)? parseFloat(order.vendor_total) :0;
            let delivery_charges =  order.delivery_charges != null && !isNaN(order.delivery_charges) ? parseFloat(order.delivery_charges) :0;
            let marketing_charges =  order.marketing_charges != null && !isNaN(order.marketing_charges) ? parseFloat(order.marketing_charges ):0;
            let order_amount = parseFloat(order.amount);

            let final =  order_amount - (vendor_total + delivery_charges + marketing_charges);

            if(index == 0){
                this.total.amount =  Math.round(order_amount);
                this.total.marketing =  Math.round(marketing_charges);
                this.total.vendor =  Math.round(vendor_total);
                this.total.delivery =  Math.round(delivery_charges);
                this.total.profit =  Math.round(final);
            }else{
                this.total.amount +=  Math.round(order_amount);
                this.total.marketing +=  Math.round(marketing_charges);
                this.total.vendor +=  Math.round(vendor_total);
            ...
            console.log(order_amount,' total:', this.total.amount);
            console.log(final,' profit:', this.total.profit);
            console.log(marketing_charges, ' marketing', this.total.marketing);
            console.log(delivery_charges, ' deivery', this.total.delivery_charges);
            console.log(vendor_total, ' vendor', this.total.vendor_total);
            return Math.round(final, 2);
        },
        addProfitProperty(){
            for(let x in this.reportData){
                // console.log(this.getProfitLoss(this.reportData[x], x));
                this.reportData[x].profit = this.getProfitLoss(this.reportData[x], x);
            }
        },
        viewOrder(order){
        ...
        downloadWithCSS() {
            ...
            html2canvas(this.$refs.report, { canvas: canvasElement }).then( (canvas) => {
                ...
                doc.save( 'Profit Report '+ this.form.from +' - '+ this.form.to+'.pdf');
