<template>
    ...
    <div class="card p-2">
        <div class="card-header"> 
            <div class="row">
                <div class="col-md-8"><h3 class="card-title ml-3">Make a Bill</h3></div>
                <div class="col-md-4 text-right"><h3 class="card-title ml-3 bg-primary p-2"><strong>Final Total :</strong> {{Math.round(calc(parseFloat(this.totalDatas.final_total)))}}</h3></div>
            </div>
        </div> 
    <!-- /.card-header -->
    <div class="card-header ">
        <h6 class="bb-1 mb-0 text-primary">Customer Details</h6>
        <div class="row ">
              <!-- <div class="col-sm-2">
                <label for="staticEmail" class="col-form-label">Canteen Number</label>
                <input type="text"  class="form-control"   v-model="cform.canteen" placeholder="Canteen Number" :class="{ 'is-invalid': errors.user_id , 'is-invalid': errors.canteen}"  >
                <span class="label label-danger" v-if="errors.user_id">{{errors.user_id}} </span>
            </div>  -->
            <div class="col-sm-2">
                <label for="staticEmail" class="col-form-label">Bukkle Number</label>
                <input type="text"  class="form-control"   v-model="cform.bukkle" placeholder="Bukkle Number" :class="{ 'is-invalid': errors.user_id }"  >
                <span class="label label-danger" v-if="errors.user_id">{{errors.user_id}} </span>
            </div> 
            <div class="col-sm-3">
                <label for="staticEmail" class="col-form-label">Select Customer Name</label>
                <!-- <input type="text"  class="form-control"   v-model="cform.customer_name" placeholder="Customer Name" :class="{ 'is-invalid': errors.user_id }"  > -->
                <v-select   v-model="cform.customer_name"  :options="userss" label="name" :class="{ 'is-invalid': form.errors.has('user_id') }" />
                <span class="label label-danger" v-if="errors.user_id">{{errors.user_id}} </span>
            </div> 
            <div class="col-sm-2">
            ...
            <div class="col-sm-1 text-left" style="margin-top: 33px!important;">
            <input type="button" value="Submit"  @click="submitcust" class="btn btn-primary btn-sm btn-outline-secondary">
            </div> 
            <div class="col-sm-2">
                <p class="mb-0 text-danger row fs-14" v-if="fetch.customer_name">Name : {{fetch.customer_name}}</p> 
                <p class="mb-0 text-danger row fs-14" v-if="fetch.bukkle"> B.Number : {{fetch.bukkle}}</p> 
                <p class="mb-0 text-danger row fs-14" v-if="fetch.mobile">Mo.Number : {{fetch.mobile}}</p>
            </div> 
            <div class="col-sm-1">
                <p class="mb-0 text-danger fs-14" v-if="fetch.customer_name" style="margin-top: 33px!important;"><input type="button" value="Make Cart Empty"  @click="CartEmpty" class="btn btn-primary btn-sm btn-outline-secondary"></p>
            </div>
    ...
    <div class="col-sm-5">
        ...
        <select class="form-control" v-model="iform.stock_select" placeholder="Select Product"  v-on:change="getstockdatanew()">
            ...
            <tr v-for="(purchase, index) in Purchases" :key="'sc'+index" :class="{ 'bg-secondary': purchase.is_active == 0  }">
                ...
                <td>
                    <div class="input-group">
                        <button @click="decrementQty(purchase)" class="btn btn-primary btn-sm">-</button>
                        <input type="text" v-model="purchase.qty" class="btn btn-primary btn-sm" readonly>
                        <button @click="incrementQty(purchase)" class="btn btn-primary btn-sm">+</button>
                    </div>
                </td>
     ...
     <div class="row mt-1">
        ...
        <hr class="bg-primary">
          <div class="row">
            ...
            <div class="col-md-1 text-center p-0"><h6 class="text-primary">:</h6></div>
            <div class="col-md-6 text-left p-0"><h6 class="text-primary">{{Math.round(calc(parseFloat(this.totalDatas.final_total)))}}</h6></div>
        </div>
        </div>
     </div>
        ...
        export default {
            ...
            methods :{
                ...
                loadOrderStatus() {
                    ...
                    decrementQty(purchase) {
                        if (purchase.qty > 1) {
                            purchase.qty--;
                            this.updatePurchaseQty(purchase);
                        }
                    },
                    // Method to increment the quantity
                    incrementQty(purchase) {
                        purchase.qty++;
                        this.updatePurchaseQty(purchase);
                    },
                    updatePurchaseQty(purchase) {
                        axios.post('/api/updatePurchaseQty', { id: purchase.id, qty: purchase.qty })
                            .then(() => {
                                // Handle the success scenario if needed
                                this.getUrl(this.iform.user_id); // Refresh the list
                            })
                            .catch(error => {
                    ...
                    getUrl(data) {   
                        var a=$('#authid').val();
                        axios.get("/api/getmakabilldata/"+a+"/user/"+data).then(({ data }) => {
                            this.Purchases = data;
                            this.getTotalData();
                        });
                    },
           ...
           }else
           {
            ...
            //     }, 1000);
                ...
                {   
                    ...
                    // this.mform.final_total=Math.round(parseFloat(this.totalDatas.final_total));
                ...
                gettotaloffinal()
                {  
                    console.log('-------last'+this.totalDatas.total_igst);
                    var vt=$('#other').val();
                    if(vt==undefined || vt==null || vt=='')
                    {
                        var vt=0;
