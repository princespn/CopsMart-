<template>
    <div class="container">
        <!-- /.row -->
        <div class="row">
            <h2> {{marketingPerson.name}} - Marketing Wallet History</h2>
        </div>
        <div class="row">
                    ...
                    <div class="card-header">
                        ...
                        <div class="card-tools">
                            ...
                            <button type="button" @click="newForm" class="btn btn-warning" data-toggle="modal"
                                data-target="#marketingPersonNew"> <i class="fa fa-marketingPerson-plus"></i> Add
                                Entry</button>
                        </div>
                    </div>
                    <!-- /.card-header -->
                        ...
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="exampleModalLongTitle"><i class="fa fa-marketingPerson"></i>
                                    {{ editMode ? 'Edit' : 'Add'}} Marketing Person Payment</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
                            <form
                                @submit.prevent="editMode ? updateMarketingPersonWalletTransaction() : createMarketingPersonWalletTransaction()">
...
<script>
    ...
    export default {
        ...
        methods :{
            createMarketingPersonWalletTransaction(){
                this.$Progress.start();
                this.form.amount = this.form.amount * -1;

                this.form.post('/api/marketing_person/'+this.marketingPersonId+'/wallet').then( ()=>{
                    Fire.$emit('LoadMarketingPersonWallet');
            ...
            updateMarketingPersonWalletTransaction(){
                this.$Progress.start();
                this.form.put('/api/marketing_person/'+this.marketingPersonId+'/wallet' + this.form.id).then( ()=>{
                    Fire.$emit('LoadMarketingPersonWallet');
                    $('#marketingPersonNew').modal('hide');
                    toast.fire({
                        type: 'success',
            ...
            loadMarketingPersonWalletList() {
                this.isLoading = true;
                axios.get("/api/marketing_person/"+this.marketingPersonId+'/wallet').then( ({ data }) => {
            ...
            newForm(){
                this.form.reset();
                this.form.marketing_person_id = this.marketingPersonId;
                this.editMode = false;
            },
            editForm(data){
                this.form.reset();
            ...
            loadMarketingPersonDetails() {
                axios.get("/api/marketing_person/"+this.marketingPersonId).then( ({ data }) => {
                    this.marketingPerson = data;
                });
            },
            loadWalletBalance() {
        ...
        mounted() {
            ...
            this.marketingPersonId = this.$route.params.marketingPersonId;
            this.loadMarketingPersonDetails();
            this.loadMarketingPersonWalletList();
            this.loadWalletBalance();
        },
        created(){
            Fire.$on('LoadMarketingPersonWallet', () => { this.loadMarketingPersonWalletList(); this.loadWalletBalance(); });
        }
