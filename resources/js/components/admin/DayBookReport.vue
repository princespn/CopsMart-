        ...
        <div class="row mt-3" >
            <div class="col-12">
                <div class="card p-1" ref="report">
                    <div class="card-header">
                        <h3 class="card-title mb-0 text-center bg-dark w-100">Day-Book Report</h3>
                    </div>
                    <!-- /.card-header -->
                    <div class="card-body table-responsive p-0 " v-if="reportData">
                        <table  v-if="!isLoading" class="table table-hover  table-bordered" >
                            <tr>
                                ...
                                <th>Final</th>
                                <th>Bank</th>
                                <th>Online (UPI/Card)</th>
                            </tr>

                            <tr v-for="order in reportData" :key="order.id">
                                <td>{{form.from | myDateFormate}}</td>
                                <td>{{form.to | myDateFormate}}</td>
                                <td>{{order.count}}</td>
                                <td>{{order.amount}}</td>
                                <td>{{order.tax}}</td>
                                <td>{{order.gst}}</td>
...
export default {
    ...
    methods :{
        getReportData(){
            ...
            this.isLoading = true;
            this.form.post('/api/report/dayreport').then( ({data})=>{
