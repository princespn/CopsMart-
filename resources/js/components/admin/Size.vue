            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title ml-3">Size</h4>
                        <div class="card-tools">
                            <button type="button" @click="newForm" class="btn btn-primary btn-sm mr-3" data-toggle="modal"
                                data-target="#categoryNew"> <i class="fa fa-plus"></i> Add Size</button>
                        </div>
                    </div>
                    <!-- /.card-header -->
...
<script>
    ...
    export default {
        ...
        methods :{
            ...
            updateCategory(){
                this.$Progress.start();
                this.form.put('api/size/' + this.form.id).then( ()=>{
                    Fire.$emit('LoadCategory');
                    $('#categoryNew').modal('hide');
                    toast.fire({
                        type: 'success',
            ...
            loadCategoryList() {
                var a =$('#authid').val();
                axios.get("api/size/"+a).then( ({ data }) => (this.categories = data) );
            },
            ...
            deleteCategory(id){
                ...
                swal.fire({
                    ...
                    if(result.value)
                    {
                        this.form.delete('/api/size/'+id).then(() => {
                            swal.fire(
                            'Deleted!',
