...
class CustomerSearchController extends Controller
{
    function suggestion(Request $request)
    {
      ...
      $silver = VendorProduct::with(['product_package'=> function($query) use ($vendor){
            if($vendor>0)
            {
        ...
        ->where('vendors.name', 'like' , '%' .$input['query'] . '%')
        ->limit(20)
        ->select(['*','vendor_products.id as vendor_product_id','products.id as product_id','vendors.name as product_name','vendor_products.is_active as is_active', 'vendor_products.is_featured as is_featured','packages.name as package_name'])
        ->groupBy('vendor_products.product_id');
      // print_r($silver);
       $silver = $silver->where($qparam)->get();
        ...
        {
            $dt=$output = array_unique( array_merge( $data , $data2 ) );
        }
        elseif(count($data) == 0 && count($data2) > 0)
        {
            $dt=$output = $data2;
        }
        elseif(count($data) > 0 && count($data2) == 0)
    ...
    function searchPagination(Request $request){
        $input = $request->input();
        if(!isset($input['query']) || !isset($input['vendor'])){
            return [];
        }
    ...
    function globalSearch(Request $request){
        ...
        $validator = \Validator::make($request->all(), ['latitude'=>'required|numeric','longitude'=>'required|numeric','search_string'=>'required']);
        if ($validator->fails()) {
           foreach($validator->errors()->toArray() as $errorvalue){
                foreach ($errorvalue as $key => $value) {
                    return response()->json(['error_message'=>$value], 422);
    ...
    function globalSearchPagination(Request $request){
        $input = $request->input();
       ...
       else
       {
            ...
            ->where('vendors.name', 'like' , '%' .$request->search_string . '%')
            ->groupBy('vendor_products.product_id')
            ->groupBy('vendor_products.vendor_id')
            ->havingRaw('distance <= vendors.service_range/1000')
            ->orderBy("distance",'asc')
